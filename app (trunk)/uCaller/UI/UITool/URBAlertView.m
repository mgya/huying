...
#define kURBAlertButtonHeight 56.0//44.0
#define kURBAlertButtonOffset 66.5//66.5
#define kURBAlertTextFieldHeight 40.0
...
-(void)removeAlertView
{
    ...
    [self hideWithAnimation:URBAlertAnimationDefault];
    [self hideWithAnimation:URBAlertAnimationDefault completionBlock:^{}];
}
...
- (void)build {
	...
	self.layer.shadowColor = [UIColor blackColor].CGColor;//黑边框的透明效果
...
- (void)layoutComponents {
    ...
    layout.buttonRect = CGRectMake(CGRectGetMinX(layoutFrame) - 15, CGRectGetMinY(layout.buttonRegionRect) + buttonRegionPadding + 12, layoutWidth + 45, buttonsHeight);
	// adjust layout frame
	...
	if (fieldCount > 0) {
		for (int i = 0; i < fieldCount; i++) {
			CGFloat yOffset = CGRectGetMinY(layout.textFieldsRect) + (kURBAlertButtonPadding + kURBAlertTextFieldHeight + 10) * (CGFloat)i;
	...
	if (count > 0) {
		...
		for (int i = 0; i < count; i++) {
			CGFloat xOffset = CGRectGetMinX(layout.buttonRect) + (0 + buttonWidth - 11) * (CGFloat)i;
			CGRect frame = CGRectIntegral(CGRectMake(xOffset, CGRectGetMinY(layout.buttonRect), buttonWidth + i*1, CGRectGetHeight(layout.buttonRect)));
			
			URBAlertViewButton *button = (URBAlertViewButton *)[self.buttons objectAtIndex:i];
			button.frame = frame;
			button.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
			button.titleLabel.textColor = [UIColor colorWithRed:17.0f/255.0f green:121.0f/255.0f blue:246.0f/255.0f alpha:1.0f];//[UIColor redColor];
...
- (UIImage *)defaultBackgroundImage {
	...
	CGContextSetFillColorWithColor(context, [UIColor colorWithRed:210.0f/255.0f green:210.0f/255.0f blue:210.0f/255.0f alpha:1.0f].CGColor);//没效果么？???
	...
	/// colors
	UIColor *gradientOuter = [UIColor colorWithRed: 255 green: 255 blue: 255 alpha: 1.0];
	UIColor *gradientInner = [UIColor colorWithRed: 255 green: 255 blue: 255 alpha: 1.0];
	...
	// main shape
	UIBezierPath *rectanglePath = [UIBezierPath bezierPathWithRect: CGRectMake(self.bounds.origin.x, self.bounds.origin.y, self.bounds.size.width, self.bounds.size.height)];
	CGContextSaveGState(context);
	[rectanglePath addClip];
    CGPoint startPoint = CGPointMake(activeBounds.size.width * 0.5f, 0.0f);
    CGPoint endPoint = CGPointMake(activeBounds.size.width * 0.5f, activeBounds.size.height);
	CGContextDrawRadialGradient(context, radialGradient,
								startPoint, self.bounds.size.width / 4,
								endPoint, self.bounds.size.width,
	...
	CGFloat buttonOffset = activeBounds.size.height - kURBAlertButtonOffset + 17;
	...
	CGContextSaveGState(context);
	CGContextSetStrokeColorWithColor(context, [UIColor colorWithRed:0.0f/255.0f green:0.0f/255.0f blue:0.0f/255.0f alpha:1.0f].CGColor);//分界线颜色
	CGContextSetShadowWithColor(context, CGSizeMake(0.0f, 0.1f), 0.0f, [UIColor colorWithRed:0.0f/255.0f green:0.0f/255.0f blue:0.0f/255.0f alpha:1.0f].CGColor);
	CGContextDrawPath(context, kCGPathStroke);
	CGContextRestoreGState(context);
    
    CGFloat buttonOffsetShu = activeBounds.size.width/2;
    CGMutablePathRef lineShuPath = CGPathCreateMutable();
	CGFloat lineShuPathY = (buttonOffsetShu - 1.0f);
	CGPathMoveToPoint(lineShuPath, NULL, lineShuPathY, linePathY);
	CGPathAddLineToPoint(lineShuPath, NULL, lineShuPathY,activeBounds.size.height);
	CGContextAddPath(context, lineShuPath);
	CGPathRelease(lineShuPath);
	CGContextSetLineWidth(context, 0.1f);
	CGContextSaveGState(context);
	CGContextSetStrokeColorWithColor(context, [UIColor colorWithRed:0.0f/255.0f green:0.0f/255.0f blue:0.0f/255.0f alpha:1.0f].CGColor);//分界线颜色
	CGContextSetShadowWithColor(context, CGSizeMake(0.0f, 0.1f), 0.0f, [UIColor colorWithRed:0.0f/255.0f green:0.0f/255.0f blue:0.0f/255.0f alpha:1.0f].CGColor);
	...
	return [image resizableImageWithCapInsetsCustom:UIEdgeInsetsMake(topCap, leftCap, bottomCap, leftCap)];
}
...
- (void)animateWithType:(URBAlertAnimation)animation show:(BOOL)show completionBlock:(void (^)())completion {
	...
	// default "pop" animation like UIAlertView
		...
		if (show) {
			...
			self.alpha = 0.0f;
			[UIView animateWithDuration:0.1 animations:^{
				self.layer.transform = CATransform3DMakeScale(1.3, 1.3, 1.0);
                //self.alpha = 0.5f;
			} completion:^(BOOL finished) {
				[UIView animateWithDuration:0.2 animations:^{
				//	self.layer.transform = CATransform3DMakeScale(1.1, 1.1, 1.0);//cz clean目前不需要
				} completion:^(BOOL finished) {
					[UIView animateWithDuration:0.2 animations:^{
						self.layer.transform = CATransform3DIdentity;
                        self.alpha = 0.9f;
			...
			self.alpha = 0.9f;
			[UIView animateWithDuration:0.2 animations:^{
				self.layer.transform = CATransform3DMakeScale(1.0, 1.0, 1.0);//cz change(1.1, 1.1, 1.0)
			} completion:^(BOOL finished) {
	...
	if (show) {
		...
		dispatch_async(dispatch_get_main_queue(), ^{
			...
			[UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionLayoutSubviews animations:^{
				self.blurredBackgroundView.alpha = 0.9f;
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		[self setBackgroundImage:normalButtonImage forState:UIControlStateHighlighted];
	}
	return self;
}
...
- (UIImage *)normalButtonImage {
	...
	// colors
    //	UIColor *buttonBgGradientColorTop = [UIColor redColor];//[UIColor colorWithRed: 0.146 green: 0.146 blue: 0.146 alpha: 1];
    //	UIColor *buttonBgGradientColorBottom = [UIColor redColor];//[UIColor colorWithRed: 0.069 green: 0.069 blue: 0.069 alpha: 1];
	...
	UIColor *buttonInnerGradientColor = [UIColor colorWithRed: 0.326 green: 0.326 blue: 0.326 alpha: 0.5];
	UIColor *buttonInnerGradientColor2 = [UIColor colorWithRed: 0.26 green: 0.26 blue: 0.26 alpha: 0.5];
	...
	CGGradientRef buttonBgGradient = nil;//CGGradientCreateWithColors(colorSpace, (__bridge CFArrayRef)buttonBgGradientColors, buttonBgGradientLocations);
	...
	CGGradientRef buttonOuterGradient = nil;//CGGradientCreateWithColors(colorSpace, (__bridge CFArrayRef)buttonOuterGradientColors, buttonOuterGradientLocations);
	...
	// shadows
	UIColor *buttonShadow = [UIColor blackColor];//左右按键的细边框
...
//	CGGradientRelease(buttonBgGradient);
    ...
    return [image resizableImageWithCapInsetsCustom:UIEdgeInsetsMake(0.0, 6.0, 0.0, 6.0)];
}
...
//	CGContextRestoreGState(context);
//	
//	[[UIColor blackColor] setStroke];//输入框边上细边框
