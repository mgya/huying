...
@interface MesCardToXMPPViewController (Private)
...
@implementation MesCardToXMPPViewController
...
-(void)contactCellMsg:(UContact*)contact
{
    ...
    [NSTimer scheduledTimerWithTimeInterval:0.6 target:self
                                   selector:@selector(returnAndSend)
                                   userInfo:nil
                                    repeats:NO];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {
        ...
        [self returnLastPage];
        if(delegate && [delegate respondsToSelector:@selector(sendCardToContact:)]){
            [delegate sendCardToContact:relayContact];
            [alertView dismissWithClickedButtonIndex:0 animated:NO];
        }
    }
}
...
-(void)returnAndSend{
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil
                                                        message:[NSString stringWithFormat:@"确定发送%@的名片到当前聊天？",relayContact.name]
                                                       delegate:self
