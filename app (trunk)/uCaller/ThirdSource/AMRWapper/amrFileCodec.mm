...
{
    size_t countNum = sizeof((unsigned char*)frameBuffer);
	memset(frameBuffer, 0, countNum);
...
//	fpamr = fopen([path cStringUsingEncoding:NSASCIIStringEncoding], "rb");
	...
	// 检查amr文件头
    size_t mStrLen = strlen(AMR_MAGIC_NUMBER);
	size_t readLen = fread(magic, sizeof(char), mStrLen, fpamr);
