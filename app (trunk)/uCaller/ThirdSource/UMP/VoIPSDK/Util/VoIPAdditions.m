...
{
    return [self VoIPSDK_base64EncodedStringWithWrapWidth:0];
}

@end


@implementation NSNotificationCenter (VoIPSDK)

- (void) VoIPSDK_postNotificationOnMainThread:(NSNotification *) notification
{
    if( pthread_main_np() ) return [self postNotification:notification];
    [self VoIPSDK_postNotificationOnMainThread:notification waitUntilDone:NO];
}

- (void) VoIPSDK_postNotificationOnMainThread:(NSNotification *) notification waitUntilDone:(BOOL) wait
{
    if( pthread_main_np() ) return [self postNotification:notification];
    [[self class] performSelectorOnMainThread:@selector( VoIPSDK__postNotification: ) withObject:notification waitUntilDone:wait];
}

+ (void) VoIPSDK__postNotification:(NSNotification *) notification
{
...
- (void) VoIPSDK_postNotificationOnMainThreadWithName:(NSString *) name object:(id) object
{
    ...
    [self VoIPSDK_postNotificationOnMainThreadWithName:name object:object userInfo:nil waitUntilDone:NO];
}
...
- (void) VoIPSDK_postNotificationOnMainThreadWithName:(NSString *) name object:(id) object userInfo:(NSDictionary *) userInfo
{
    ...
    [self VoIPSDK_postNotificationOnMainThreadWithName:name object:object userInfo:userInfo waitUntilDone:NO];
}
...
- (void) VoIPSDK_postNotificationOnMainThreadWithName:(NSString *) name object:(id) object userInfo:(NSDictionary *) userInfo waitUntilDone:(BOOL) wait
{
    ...
    [[self class] performSelectorOnMainThread:@selector( VoIPSDK__postNotificationName: ) withObject:info waitUntilDone:wait];
}

+ (void) VoIPSDK__postNotificationName:(NSDictionary *) info
{
...
@implementation NSString (VoIPSDK)

+ (NSString *)VoIPSDK_stringWithBase64EncodedString:(NSString *)string
{
    NSData *data = [NSData VoIPSDK_dataWithBase64EncodedString:string];
    if (data)
    {
...
- (NSString *)VoIPSDK_base64EncodedStringWithWrapWidth:(NSUInteger)wrapWidth
{
    ...
    return [data VoIPSDK_base64EncodedStringWithWrapWidth:wrapWidth];
}

- (NSString *)VoIPSDK_base64EncodedString
{
    NSData *data = [self dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:YES];
    return [data VoIPSDK_base64EncodedString];
}

- (NSString *)VoIPSDK_base64DecodedString
{
    return [NSString VoIPSDK_stringWithBase64EncodedString:self];
}

- (NSData *)VoIPSDK_base64DecodedData
{
    return [NSData VoIPSDK_dataWithBase64EncodedString:self];
}
...
@implementation NSDictionary (VoIPSDK)
