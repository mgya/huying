#import "TZ_DDXMLPrivate.h"
#import "NSString+TZ_DDXML.h"
...
@implementation TZ_DDXMLNode

static void MyErrorHandler(void * userData, xmlErrorPtr error);

#if TZ_DDXML_DEBUG_MEMORY_ISSUES
...
static void MarkBirth(void *xmlPtr, TZ_DDXMLNode *wrapper);
static void MarkDeath(void *xmlPtr, TZ_DDXMLNode *wrapper);
...
+ (void)initialize
{
	...
	dispatch_once(&onceToken, ^{
		...
		#if TZ_DDXML_DEBUG_MEMORY_ISSUES
		{
			...
			zombieQueue = dispatch_queue_create("TZ_DDXMLZombieQueue", NULL);
		}
		#endif
...
{
	return [[TZ_DDXMLElement alloc] initWithName:name];
}

+ (id)elementWithName:(NSString *)name stringValue:(NSString *)string
{
	return [[TZ_DDXMLElement alloc] initWithName:name stringValue:string];
}

+ (id)elementWithName:(NSString *)name children:(NSArray *)children attributes:(NSArray *)attributes
{
	TZ_DDXMLElement *result = [[TZ_DDXMLElement alloc] initWithName:name];
...
{
	return [[TZ_DDXMLElement alloc] initWithName:name URI:URI];
}
...
+ (id)attributeWithName:(NSString *)name stringValue:(NSString *)stringValue
{
	...
	return [[TZ_DDXMLAttributeNode alloc] initWithAttrPrimitive:attr owner:nil];
}
...
+ (id)attributeWithName:(NSString *)name URI:(NSString *)URI stringValue:(NSString *)stringValue
{
	...
	TZ_DDXMLAttributeNode *result = [[TZ_DDXMLAttributeNode alloc] initWithAttrPrimitive:attr owner:nil];
	[result setURI:URI];
...
+ (id)namespaceWithName:(NSString *)name stringValue:(NSString *)stringValue
{
	...
	return [[TZ_DDXMLNamespaceNode alloc] initWithNsPrimitive:ns nsParent:NULL owner:nil];
}
...
+ (id)processingInstructionWithName:(NSString *)name stringValue:(NSString *)stringValue
{
	...
	return [[TZ_DDXMLNode alloc] initWithPrimitive:(xmlKindPtr)procInst owner:nil];
}
...
+ (id)commentWithStringValue:(NSString *)stringValue
{
	...
	return [[TZ_DDXMLNode alloc] initWithPrimitive:(xmlKindPtr)comment owner:nil];
}
...
+ (id)textWithStringValue:(NSString *)stringValue
{
	...
	return [[TZ_DDXMLNode alloc] initWithPrimitive:(xmlKindPtr)text owner:nil];
}
...
+ (id)nodeWithUnknownPrimitive:(xmlKindPtr)kindPtr owner:(TZ_DDXMLNode *)owner
{
	if (kindPtr->type == XML_DOCUMENT_NODE)
	{
		return [TZ_DDXMLDocument nodeWithDocPrimitive:(xmlDocPtr)kindPtr owner:owner];
	}
	else if (kindPtr->type == XML_ELEMENT_NODE)
	{
		return [TZ_DDXMLElement nodeWithElementPrimitive:(xmlNodePtr)kindPtr owner:owner];
	}
	else if (kindPtr->type == XML_NAMESPACE_DECL)
	{
		// Todo: This may be a problem...
		
		return [TZ_DDXMLNamespaceNode nodeWithNsPrimitive:(xmlNsPtr)kindPtr nsParent:NULL owner:owner];
	}
	else if (kindPtr->type == XML_ATTRIBUTE_NODE)
	{
		return [TZ_DDXMLAttributeNode nodeWithAttrPrimitive:(xmlAttrPtr)kindPtr owner:owner];
	}
	else
	{
		return [TZ_DDXMLNode nodeWithPrimitive:kindPtr owner:owner];
	}
}

/**
 * Returns a TZ_DDXML wrapper object for the given primitive node.
 * The given node MUST be non-NULL and of the proper type.
**/
+ (id)nodeWithPrimitive:(xmlKindPtr)kindPtr owner:(TZ_DDXMLNode *)owner
{
	return [[TZ_DDXMLNode alloc] initWithPrimitive:kindPtr owner:owner];
}
 ...
 * The given node MUST be non-NULL and of the proper type.
**/
- (id)initWithPrimitive:(xmlKindPtr)kindPtr owner:(TZ_DDXMLNode *)inOwner
{
	if ((self = [super init]))
	{
		genericPtr = kindPtr;
...
- (id)init
{
	...
	if ([self isKindOfClass:[TZ_DDXMLInvalidNode class]])
	{
		return self;
	}
	else
	{
		return [[TZ_DDXMLInvalidNode alloc] init];
	}
}
...
- (void)dealloc
{
	...
	// This may be the case if, e.g., TZ_DDXMLElement calls [self release] from it's init method.
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	if (IsXmlDocPtr(genericPtr))
	{
		...
		return [[TZ_DDXMLDocument alloc] initWithDocPrimitive:copyDocPtr owner:nil];
	}
	...
	if (IsXmlNodePtr(genericPtr))
	{
		...
		if ([self isKindOfClass:[TZ_DDXMLElement class]])
			return [[TZ_DDXMLElement alloc] initWithElementPrimitive:copyNodePtr owner:nil];
		else
			return [[TZ_DDXMLNode alloc] initWithPrimitive:(xmlKindPtr)copyNodePtr owner:nil];
	}
	...
	if (IsXmlAttrPtr(genericPtr))
	{
		...
		return [[TZ_DDXMLAttributeNode alloc] initWithAttrPrimitive:copyAttrPtr owner:nil];
	}
	...
	if (IsXmlNsPtr(genericPtr))
	{
		...
		return [[TZ_DDXMLNamespaceNode alloc] initWithNsPrimitive:copyNsPtr nsParent:NULL owner:nil];
	}
	...
	if (IsXmlDtdPtr(genericPtr))
	{
		...
		return [[TZ_DDXMLNode alloc] initWithPrimitive:(xmlKindPtr)copyDtdPtr owner:nil];
	}
...
{
	// TZ_DDXMLNode, TZ_DDXMLElement, and TZ_DDXMLDocument are simply light-weight wrappers atop a libxml structure.
	// 
	// To provide maximum speed and thread-safety,
	// multiple TZ_DDXML wrapper objects may be created that wrap the same underlying libxml node.
	// 
	// Thus equality is simply a matter of what underlying libxml node TZ_DDXML is wrapping.
	
	if ([anObject class] == [self class])
	{
		TZ_DDXMLNode *aNode = (TZ_DDXMLNode *)anObject;
...
- (TZ_DDXMLNodeKind)kind
{
	...
	if (genericPtr != NULL)
		...
		return TZ_DDXMLInvalidKind;
}

- (void)setName:(NSString *)name
{
	// Note: TZ_DDXMLNamespaceNode overrides this method
...
{
	// Note: TZ_DDXMLNamespaceNode overrides this method
	// Note: TZ_DDXMLAttributeNode overrides this method
...
/**
 * Returns the TZ_DDXMLDocument object containing the root element and representing the XML document as a whole.
...
- (TZ_DDXMLDocument *)rootDocument
{
	...
	if (node == NULL || node->doc == NULL)
		...
		return [TZ_DDXMLDocument nodeWithDocPrimitive:node->doc owner:self];
}
...
- (TZ_DDXMLNode *)parent
{
	...
	if (node->parent == NULL)
		...
		return [TZ_DDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)node->parent owner:self];
}
...
/**
 * Returns an immutable array containing the child nodes of the receiver (as TZ_DDXMLNode objects).
**/
- (NSArray *)children
{
	// Note: TZ_DDXMLNamespaceNode overrides this method
	...
	while (child != NULL)
	{
		[result addObject:[TZ_DDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)child owner:self]];
 ...
 * Returns a TZ_DDXMLNode object or nil if the receiver has no children.
 * 
 * If the receive has children and index is out of bounds, an exception is raised.
 * 
 * The receiver should be a TZ_DDXMLNode object representing a document, element, or document type declaration.
...
- (TZ_DDXMLNode *)childAtIndex:(NSUInteger)index
{
	...
	while (child != NULL)
	{
		if (i == index)
		{
			return [TZ_DDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)child owner:self];
		}
	...
	TZ_DDXMLAssert(NO, @"index (%u) beyond bounds (%u)", (unsigned)index, (unsigned)i);
	
	return nil;
}

/**
 * Returns the previous TZ_DDXMLNode object that is a sibling node to the receiver.
 * 
...
- (TZ_DDXMLNode *)previousSibling
{
	...
	if (node->prev == NULL)
		...
		return [TZ_DDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)node->prev owner:self];
}

/**
 * Returns the next TZ_DDXMLNode object that is a sibling node to the receiver.
 * 
...
- (TZ_DDXMLNode *)nextSibling
{
	...
	if (node->next == NULL)
		...
		return [TZ_DDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)node->next owner:self];
}

/**
 * Returns the previous TZ_DDXMLNode object in document order.
 * 
 ...
 * nil is returned. TZ_DDXMLNode bypasses namespace and attribute nodes when it traverses a tree in document order.
**/
- (TZ_DDXMLNode *)previousNode
{
	...
	if (previousSibling != NULL)
	{
		if (previousSibling->last != NULL)
		{
			...
			return [TZ_DDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)lastChild owner:self];
		}
		else
		{
			// The previous sibling has no children, so the previous node is simply the previous sibling
			return [TZ_DDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)previousSibling owner:self];
		}
	}
...
/**
 * Returns the next TZ_DDXMLNode object in document order.
 * 
 ...
 * TZ_DDXMLNode bypasses namespace and attribute nodes when it traverses a tree in document order.
**/
- (TZ_DDXMLNode *)nextNode
{
	...
	TZ_DDXMLNode *firstChild = [self childAtIndex:0];
	if (firstChild)
		return firstChild;
	
	// If the node has a next sibling, then next node is the same as next sibling
	
	TZ_DDXMLNode *nextSibling = [self nextSibling];
	if (nextSibling)
		return nextSibling;
	...
	while (parent != NULL)
	{
		...
		if (parentNextSibling != NULL)
			return [TZ_DDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)parentNextSibling owner:self];
		else
 ...
 *
 * This method is applicable to TZ_DDXMLNode objects representing elements, text, comments, processing instructions,
...
- (NSString *)XMLStringWithOptions:(NSUInteger)options
{
	...
	if (options & TZ_DDXMLNodeCompactEmptyElement)
		xmlSaveNoEmptyTags = 0;
	else
		xmlSaveNoEmptyTags = 1;
	
	int format = 0;
	if (options & TZ_DDXMLNodePrettyPrint)
	{
		format = 1;
	...
	else
	{
		...
		else
		{
			...
			for (i = 0; i < count; i++)
			{
				...
				[mResult addObject:[TZ_DDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)node owner:self]];
			}
...
// 
// TZ_DDXMLElement *root = [[TZ_DDXMLElement alloc] initWithXMLString:str error:nil];
// NSArray *children = [root children];
// 
// dispatch_queue_t q = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
// dispatch_apply([children count], q, ^(size_t i) {
//     TZ_DDXMLElement *child = [children objectAtIndex:i];
//     <process child>
// });
// 
// 
...
// 
// TZ_DDXMLElement *root = [[TZ_DDXMLElement alloc] initWithXMLString:str error:nil];
// TZ_DDXMLElement *child = [root elementForName:@"starbucks"];
// 
 ...
 * Errors are caught in the MyErrorHandler method within TZ_DDXMLDocument.
**/
+ (NSError *)lastError
{
	...
	if(lastErrorValue)
	{
		...
		return [NSError errorWithDomain:@"TZ_DDXMLErrorDomain" code:errCode userInfo:info];
	}
	else
	{
		return nil;
...
static void MyErrorHandler(void * userData, xmlErrorPtr error)
{
	...
	// This ensure's thread safey, and easy access for all other TZ_DDXML classes.
...
// and the values are mutable arrays. Any wrapper objects (TZ_DDXMLElement, TZ_DDXMLNode, etc) get added to the
...
static void RecursiveMarkZombiesFromNode(xmlNodePtr node)
{
	// This method only exists if TZ_DDXML_DEBUG_MEMORY_ISSUES is enabled.
...
BOOL TZ_DDXMLIsZombie(void *xmlPtr, TZ_DDXMLNode *wrapper)
{
	// This method only exists if TZ_DDXML_DEBUG_MEMORY_ISSUES is enabled.
...
@implementation TZ_DDXMLNamespaceNode

/**
 * Returns a TZ_DDXML wrapper object for the given primitive node.
 * The given node MUST be non-NULL and of the proper type.
**/
+ (id)nodeWithNsPrimitive:(xmlNsPtr)ns nsParent:(xmlNodePtr)parent owner:(TZ_DDXMLNode *)owner
{
	return [[TZ_DDXMLNamespaceNode alloc] initWithNsPrimitive:ns nsParent:parent owner:owner];
}
 ...
 * The given node MUST be non-NULL and of the proper type.
**/
- (id)initWithNsPrimitive:(xmlNsPtr)ns nsParent:(xmlNodePtr)parent owner:(TZ_DDXMLNode *)inOwner
{
...
- (TZ_DDXMLNode *)parent
{
	...
	if (nsParentPtr == NULL)
		...
		return [TZ_DDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)nsParentPtr owner:self];
}
...
- (void)detach
{
	...
	if (nsParentPtr != NULL)
	{
		[TZ_DDXMLNode detachNamespace:(xmlNsPtr)genericPtr fromNode:nsParentPtr];
...
@implementation TZ_DDXMLAttributeNode

+ (id)nodeWithAttrPrimitive:(xmlAttrPtr)attr owner:(TZ_DDXMLNode *)owner
{
	return [[TZ_DDXMLAttributeNode alloc] initWithAttrPrimitive:attr owner:owner];
}

- (id)initWithAttrPrimitive:(xmlAttrPtr)attr owner:(TZ_DDXMLNode *)inOwner
{
...
- (void)detach
{
	...
	if (attr->parent != NULL)
	{
		...
		//           so we need to use TZ_DDXMLAttributeNode's attrNsPtr.
...
- (void)setURI:(NSString *)URI
{
	...
	// There is no way, within the API, to get a TZ_DDXMLNode wrapper for the attribute's namespace.
	
	// Remember: attr->ns is simply a pointer to a namespace owned by somebody else.
	//           Unless that points to our attrNsPtr (defined in TZ_DDXMLAttributeNode) we cannot free it.
...
@implementation TZ_DDXMLInvalidNode
