...
@implementation TZ_DDXMLElement

/**
 * Returns a TZ_DDXML wrapper object for the given primitive node.
 * The given node MUST be non-NULL and of the proper type.
**/
+ (id)nodeWithElementPrimitive:(xmlNodePtr)node owner:(TZ_DDXMLNode *)owner
{
	return [[TZ_DDXMLElement alloc] initWithElementPrimitive:node owner:owner];
}

- (id)initWithElementPrimitive:(xmlNodePtr)node owner:(TZ_DDXMLNode *)inOwner
{
...
- (id)initWithName:(NSString *)name URI:(NSString *)URI
{
	...
	TZ_DDXMLElement *result = [self initWithElementPrimitive:node owner:nil];
	[result setURI:URI];
...
{
	TZ_DDXMLDocument *doc = [[TZ_DDXMLDocument alloc] initWithXMLString:string options:0 error:error];
	if (doc == nil)
	{
		return nil;
	}
	
	TZ_DDXMLElement *result = [doc rootElement];
	[result detach];
                          ...
                          uri:(NSString *)uri
{
	...
	while (child)
	{
		if (IsXmlNodePtr(child))
		{
			...
			if (match)
			{
				[result addObject:[TZ_DDXMLElement nodeWithElementPrimitive:child owner:self]];
			}
		}
...
/**
 * Returns the child element nodes (as TZ_DDXMLElement objects) of the receiver that have a specified name.
 * 
...
- (NSArray *)elementsForName:(NSString *)name
{
	...
	[TZ_DDXMLNode getPrefix:&prefix localName:&localName forName:name];
...
- (NSArray *)elementsForLocalName:(NSString *)localName URI:(NSString *)uri
{
	...
	else
	{
		...
		[TZ_DDXMLNode getPrefix:&prefix localName:&realLocalName forName:localName];
		
		return [self _elementsForName:localName localName:realLocalName prefix:prefix uri:uri];
	}
}

- (TZ_DDXMLElement *)elementForName:(NSString *)name
{
	...
	else
	{
		...
		//
		// --!!-- This bug does NOT exist in TZ_DDXML --!!--
...
- (void)_removeAttributeForName:(NSString *)name
{
	...
	if (attr)
	{
		...
		do
		{
			if (xmlStrEqual(attr->name, xmlName))
			{
				[TZ_DDXMLNode removeAttribute:attr];
				return;
			}
			attr = attr->next;
...
- (void)addAttribute:(TZ_DDXMLNode *)attribute
{
#if TZ_DDXML_DEBUG_MEMORY_ISSUES
	TZ_DDXMLNotZombieAssert();
#endif
	...
	TZ_DDXMLAssert([attribute _hasParent] == NO, @"Cannot add an attribute with a parent; detach or copy first");
	TZ_DDXMLAssert(IsXmlAttrPtr(attribute->genericPtr), @"Not an attribute");
...
- (NSArray *)attributes
{
	...
	while (attr != NULL)
	{
		[result addObject:[TZ_DDXMLAttributeNode nodeWithAttrPrimitive:attr owner:self]];
...
- (TZ_DDXMLNode *)attributeForName:(NSString *)name
{
#if TZ_DDXML_DEBUG_MEMORY_ISSUES
	TZ_DDXMLNotZombieAssert();
#endif
	...
	if (attr)
	{
		...
		do
		{
			if (attr->ns && attr->ns->prefix)
			{
				...
				{
					return [TZ_DDXMLAttributeNode nodeWithAttrPrimitive:attr owner:self];
				}
			}
			else
			{
...
- (void)setAttributes:(NSArray *)attributes
{
	...
	[TZ_DDXMLNode removeAllAttributesFromNode:(xmlNodePtr)genericPtr];
	
	NSUInteger i;
	for (i = 0; i < [attributes count]; i++)
	{
		TZ_DDXMLNode *attribute = [attributes objectAtIndex:i];
...
- (void)_removeNamespaceForPrefix:(NSString *)name
{
	...
	while (ns != NULL)
	{
		...
		{
			[TZ_DDXMLNode removeNamespace:ns fromNode:node];
			break;
		}
		ns = ns->next;
	}
...
- (void)_addNamespace:(TZ_DDXMLNode *)namespace
{
	// NSXML version uses this same assertion
	TZ_DDXMLAssert([namespace _hasParent] == NO, @"Cannot add a namespace with a parent; detach or copy first");
	TZ_DDXMLAssert(IsXmlNsPtr(namespace->genericPtr), @"Not a namespace");
	...
	if ([namespace isKindOfClass:[TZ_DDXMLNamespaceNode class]])
	{
		TZ_DDXMLNamespaceNode *ddNamespace = (TZ_DDXMLNamespaceNode *)namespace;
...
- (void)addNamespace:(TZ_DDXMLNode *)namespace
{
#if TZ_DDXML_DEBUG_MEMORY_ISSUES
	TZ_DDXMLNotZombieAssert();
#endif
...
- (NSArray *)namespaces
{
	...
	while (ns != NULL)
	{
		[result addObject:[TZ_DDXMLNamespaceNode nodeWithNsPrimitive:ns nsParent:(xmlNodePtr)genericPtr owner:self]];
...
- (TZ_DDXMLNode *)namespaceForPrefix:(NSString *)prefix
{
#if TZ_DDXML_DEBUG_MEMORY_ISSUES
	TZ_DDXMLNotZombieAssert();
#endif
	...
	if ([prefix length] == 0)
	{
		...
		if (ns != NULL)
		{
			return [TZ_DDXMLNamespaceNode nodeWithNsPrimitive:ns nsParent:(xmlNodePtr)genericPtr owner:self];
		}
	}
	else
	{
...
- (void)setNamespaces:(NSArray *)namespaces
{
	...
	[TZ_DDXMLNode removeAllNamespacesFromNode:(xmlNodePtr)genericPtr];
	
	NSUInteger i;
	for (i = 0; i < [namespaces count]; i++)
	{
		TZ_DDXMLNode *namespace = [namespaces objectAtIndex:i];
		[self _addNamespace:namespace];
...
**/
- (TZ_DDXMLNode *)_recursiveResolveNamespaceForPrefix:(NSString *)prefix atNode:(xmlNodePtr)nodePtr
{
	// This is a private/internal method
	...
	if (ns)
	{
		...
		do
		{
			if (xmlStrEqual(ns->prefix, xmlPrefix))
			{
				return [TZ_DDXMLNamespaceNode nodeWithNsPrimitive:ns nsParent:nodePtr owner:self];
			}
			ns = ns->next;
...
**/
- (TZ_DDXMLNode *)resolveNamespaceForName:(NSString *)name
{
#if TZ_DDXML_DEBUG_MEMORY_ISSUES
	TZ_DDXMLNotZombieAssert();
#endif
...
- (void)addChild:(TZ_DDXMLNode *)child
{
	...
	TZ_DDXMLAssert([child _hasParent] == NO, @"Cannot add a child that has a parent; detach or copy first");
	TZ_DDXMLAssert(IsXmlNodePtr(child->genericPtr),
...
- (void)removeChildAtIndex:(NSUInteger)index
{
	...
	while (child != NULL)
	{
		...
		if (IsXmlNodePtr(child))
		{
			if (i == index)
			{
				[TZ_DDXMLNode removeChild:child];
				return;
			}
...
- (void)setChildren:(NSArray *)children
{
	...
	[TZ_DDXMLNode removeAllChildrenFromNode:(xmlNodePtr)genericPtr];
	
	NSUInteger i;
	for (i = 0; i < [children count]; i++)
	{
		TZ_DDXMLNode *child = [children objectAtIndex:i];
		[self addChild:child];
