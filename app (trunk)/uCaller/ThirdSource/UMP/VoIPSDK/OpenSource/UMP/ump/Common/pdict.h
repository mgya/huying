...
#ifndef __UMPStack__pdict__
#define __UMPStack__pdict__
...
protected:
     ...
     */
    PINLINE PBOOL AbstractContains(
    ...
    public:
        ...
        PBOOL SetLastElementAt(PINDEX index);
        Element * GetElementAt(const PObject & key);
        PINDEX GetElementsIndex(const PObject*obj,PBOOL byVal,PBOOL keys) const;
...
public:
     ...
     */
    inline PSet(PBOOL initialDeleteObjects = FALSE)
...
public: \
cls(PBOOL initialDeleteObjects = initDelObj) \
...
{ reference->deleteObjects = c->reference->deleteObjects; } \
public: \
inline cls(PBOOL initialDeleteObjects = FALSE) \
: PAbstractSet() { AllowDeleteObjects(initialDeleteObjects); } \
virtual PObject * Clone() const \
{ return PNEW cls(0, this); } \
...
{ Remove(key); } \
inline PBOOL operator[](const K & key) const \
{ return AbstractContains(key); } \
inline PBOOL Contains(const K & key) const \
{ return AbstractContains(key); } \
...
: cls##_PTemplate(dummy, c) { } \
public: \
inline cls(PBOOL initialDeleteObjects = initDelObj) \
...
virtual PBOOL SetDataAt(PINDEX index, PINDEX ordinal) \
{ return PAbstractDictionary::SetDataAt(index, PNEW POrdinalKey(ordinal)); } \
virtual PBOOL SetAt(const K & key, PINDEX ordinal) \
{ return AbstractSetAt(key, PNEW POrdinalKey(ordinal)); } \
...
#endif /* defined(__UMPStack__pdict__) */
