...
void PBase64::StartEncoding(PBOOL useCRLF)
{
    encodedString = "";
...
PString PBase64::CompleteEncoding()
{
    ...
    switch (saveCount) {
        ...
        case 3 :
            *out++ = Binary2Base64[saveTriple[0] >> 2];
            *out++ = Binary2Base64[((saveTriple[0]&3)<<4) | (saveTriple[1]>>4)];
            *out++ = Binary2Base64[((saveTriple[1]&15)<<2) | (saveTriple[2]>>6)];
            *out++ = Binary2Base64[((saveTriple[2]&63))];
            break;
...
PBOOL PBase64::ProcessDecoding(const PString & str)
{
    return ProcessDecoding((const char *)str);
}


PBOOL PBase64::ProcessDecoding(const char * cstr)
{
...
PBOOL PBase64::GetDecodedData(void * dataBlock, PINDEX length)
{
    perfectDecode = quadPosition == 0;
    PBOOL bigEnough = length >= decodeSize;
...
PBOOL PBase64::Decode(const PString & str, PBYTEArray & data)
{
    PBase64 decoder;
...
PBOOL PBase64::Decode(const PString & str, void * dataBlock, PINDEX length)
{
    PBase64 decoder;
    decoder.ProcessDecoding(str);
...
PBOOL PCypher::Decode(const PString & cypher, PString & clear)
{
    clear = PString();
...
PBOOL PCypher::Decode(const PString & cypher, PBYTEArray & clear)
{
    PBYTEArray coded;
...
PBOOL PCypher::Decode(const PBYTEArray & coded, PBYTEArray & clear)
{
    PAssert((blockSize%8) == 0, PUnsupportedFeature);
...
void PTEACypher::Initialise(PBOOL)
{
...
PBOOL PSecureConfig::ValidatePending()
{
