...
#include "include/channel_transport.h"
...
VoiceChannelTransport::VoiceChannelTransport(VoENetwork* voe_network,
                                             int channel,
                                             URTPEncryption* urtp_encryption)
    : channel_(channel),
      voe_network_(voe_network),
      urtp_encryption_(urtp_encryption),
      pt_(0),
      _decryptionRTPBufferPtr(NULL){
  uint8_t socket_threads = 1;
  socket_transport_ = UdpTransport::Create(channel, socket_threads,urtp_encryption);
...
void VoiceChannelTransport::IncomingRTPPacket(
    ...
    //add by huach
    if(!_decryptionRTPBufferPtr){
        _decryptionRTPBufferPtr = new int8_t[1500];
    }
    GIPS_Word32 decryptedBufferLength = 0;
    urtp_encryption_->decrypt(channel_,
                            (unsigned char *)incoming_rtp_packet,
                            (unsigned char *)_decryptionRTPBufferPtr,
                            packet_length,
                            (int*)&decryptedBufferLength);
    
//    U_INFO("recv urtp");
    
//    if(!is_call_connected_)
//        return;
    
    gips::ModuleRTPUtility::URTPUtility urtpUtil((GIPS_UWord8*)_decryptionRTPBufferPtr,decryptedBufferLength);
    urtpUtil.ToRTP((GIPS_UWord8**)&_decryptionRTPBufferPtr,1234567890,&decryptedBufferLength,pt_);
    
  voe_network_->ReceivedRTPPacket(
      channel_, _decryptionRTPBufferPtr, decryptedBufferLength, PacketTime());
}
...
int VoiceChannelTransport::SetSendDestination(const char* ip_address,
  ...
  return socket_transport_->InitializeSendSockets(ip_address, rtp_port);
    ...
    void VoiceChannelTransport::SetPT(int pt){
        pt_=pt;
    }
    
    void VoiceChannelTransport::SetCallConnected(bool is_call_connected){
        is_call_connected_ = is_call_connected;
    }
