...
class VoiceChannelTransport : public UdpTransportData {
 public:
  VoiceChannelTransport(VoENetwork* voe_network, int channel,URTPEncryption* urtp_encryption);
  ...
  void SetPT(int pt);
  void SetCallConnected(bool is_call_connected);

 private:
  int channel_;
  VoENetwork* voe_network_;
  UdpTransport* socket_transport_;
  URTPEncryption* urtp_encryption_;
  int pt_;
  bool is_call_connected_;
    
private:
    int8_t* _decryptionRTPBufferPtr;
    //int8_t* _decryptionRTPBufferPtr;
};
