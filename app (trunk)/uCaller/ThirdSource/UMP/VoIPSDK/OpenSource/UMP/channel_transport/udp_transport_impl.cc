...
UdpTransport* UdpTransport::Create(const int32_t id,
                                   uint8_t& numSocketThreads,URTPEncryption* urtp_encryption)
{
  return new UdpTransportImpl(id,
                              new SocketFactory(),
                              UdpSocketManager::Create(id, numSocketThreads),urtp_encryption);
}
...
UdpTransportImpl::UdpTransportImpl(const int32_t id,
    ...
    : _id(id),
      ...
      _rtcpFilterPort(0),
      _packetCallback(0),
      _urtp_encryption(urtp_encryption),
...
//    U_INFO("send urtp");
    //add by huach
    GIPS_Word32 new_len;
    gips::ModuleRTPUtility::URTPUtility urtpUtil((const uint8_t*)data,length);
    urtpUtil.ToURTP((GIPS_UWord8**)&data,_id,&new_len);
    
    
    
    if(!_encryptionRTPBufferPtr){
        _encryptionRTPBufferPtr = new int8_t[1500];
    }
    GIPS_Word32 en_len = 0;
    _urtp_encryption->encrypt(_id, (unsigned char *)data, (unsigned char *)_encryptionRTPBufferPtr, new_len, &en_len);

//    U_INFO("send urtp,length:" << length << ",urtplen:" << new_len << ",enlen:" << en_len);
    
    if(_ptrSendRtpSocket)
    {
        return _ptrSendRtpSocket->SendTo((const int8_t*)_encryptionRTPBufferPtr, en_len,
                                         _remoteRTPAddr);

    } else if(_ptrRtpSocket)
    {
        return _ptrRtpSocket->SendTo((const int8_t*)_encryptionRTPBufferPtr, en_len,
                                     _remoteRTPAddr);
    }
    return -1;
}
